...
const happPath = path.join(__dirname, "../../workdir/branchy.happ")
...
test("branchy basic tests", async () => {
  await runScenario(async (scenario: Scenario) => {
    ...
    try {
      await alice_branchy.callZome({zome_name:'branchy', fn_name:'initialize', payload: {units: [["_alive", rootUnit]]}} );
    }
    catch (e) {
      console.log("Error in initialize", e)
    }

    let units:any = await alice_branchy.callZome({zome_name:'branchy',fn_name:'get_units'} );
    ...
    const unit1Output:any = await alice_branchy.callZome({zome_name:'branchy', fn_name:'create_unit', payload: {state: "_alive", unit:unit1}} );
    assert.ok(unit1Output)
    const unit1Hash = encodeHashToBase64(unit1Output.info.hash)
    console.log("unit1Hash", unit1Hash);

    units  = await alice_branchy.callZome({zome_name:'branchy', fn_name:'get_units'} );
