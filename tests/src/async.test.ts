...
test("async tests", async () => {
  await runScenario(async (scenario: Scenario) => {
      ...
      (unitHash: ActionHash) =>
        retryUntilSuccess(
          async () => {
            ...
            let unitRecord: Record = await alice_branchy.callZome({zome_name:'branchy', fn_name:'get_unit', payload: unitHash} );
    ...
    const getUnits = async () :Promise<Link[]> => {
      ...
      return alice_branchy.callZome({zome_name:'branchy', fn_name:'get_unit_links'} );
    }
    const allUnits = pipe(
      liveLinksStore(
        () => getUnits(),
