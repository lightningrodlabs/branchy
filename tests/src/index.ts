...
const dnaPath = path.join(__dirname, "../../apps/launcher/dnas/branchy/branchy.dna")
...
test("branchy basic tests", async (t) => {
...
try {
    ...
    const [alice_branchy] = alice.cells;
    const [bobbo_branchy] = bobbo.cells;
    ...
    let rootUnit = {
      ...
      description: "Holochain Community Standards",
      version: "vidx:0",
      name: "", // max 10 char
...
//      t.deepEqual(signal.data.payload.message, { type: 'NewUnit', content: unit1})
 //   })
    try {
      await alice_branchy.callZome({zome_name:'branchy', fn_name:'initialize', payload: {units: [["_alive", rootUnit]], documents:[initializer]}} );
    }
    catch (e) {
      console.log("Error in initialize", e)
    }

    let docs:any = await alice_branchy.callZome({zome_name:'branchy',fn_name:'get_documents', payload:""} );
    ...
    const unit1Output:any = await alice_branchy.callZome({zome_name:'branchy', fn_name:'create_unit', payload: unit1} );
    t.ok(unit1Output)
    ...
    const document1Hash = await alice_branchy.callZome({zome_name:'branchy', fn_name:'create_document', payload: {path:doc1Path, document:document1}} );
    t.ok(document1Hash)
    console.log("document1Hash", document1Hash);

    docs = await alice_branchy.callZome({zome_name:'branchy',fn_name:'get_documents', payload: doc1Path} );
    t.equal(docs.length, 1)
    t.equal(docs[0].hash, document1Hash)

    const units :Array<any> = await alice_branchy.callZome({zome_name:'branchy', fn_name:'get_units'} );
    ...
    let newDocHash
    try {
    let tree:any = await alice_branchy.callZome({zome_name:'branchy', fn_name:'get_tree'} );
    ...
    console.log("document1Hash", document1Hash)
    newDocHash = await alice_branchy.callZome({zome_name:'branchy', fn_name:'update_document', payload: {hash: document1Hash, document: document1, path: doc1Path}} );
    tree = await alice_branchy.callZome({zome_name:'branchy', fn_name:'get_tree',} );
    ...
    const markActionHashes = await alice_branchy.callZome({zome_name:'branchy', fn_name:'mark_document', payload: [{hash: newDocHash, markType: 1, mark:"good"}]} );
    docs = await alice_branchy.callZome({zome_name:'branchy',fn_name:'get_documents', payload:doc1Path} );
    ...
    try {
      newDocHash = await alice_branchy.callZome({zome_name:'branchy', fn_name:'advance_state', payload: {newState: "align", unitHash: unit1Hash, documentHash: document1Hash, document: document1}} );
      const tree:any = await alice_branchy.callZome({zome_name:'branchy', fn_name:'get_tree',} );
    ...
    const deleteActionHash : ActionHash = await alice_branchy.callZome({zome_name:'branchy', fn_name:'delete_document', payload: docs[2].actions[0].hash} );
    docs = await alice_branchy.callZome({zome_name:'branchy',fn_name:'get_documents', payload:doc1Path} );
