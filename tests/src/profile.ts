...
export default (orchestrator) => {
  ...
  orchestrator.registerScenario('test profile zomes', async (s, t) => {
    ...
    const [alice_branchy_happ, bob_branchy_happ] = await installAgents(conductor,  ["alice", "bobbo"])
    const [alice] = alice_branchy_happ.cells
    const [bobbo] = bob_branchy_happ.cells

    // Trigger init to run in both zomes
    await alice.call('branchy', 'list_channels', { category: "General" });
    await bobbo.call('branchy', 'list_channels', { category: "General" });
