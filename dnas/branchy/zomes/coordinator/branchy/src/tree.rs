...
fn get_entry_hashes(path: &Path) -> ExternResult<Vec<UnitInfo>> {
    ...
    for l in links {
        ...
        let target = l.target.try_into().map_err(|e| BranchyError::HashConversionError)?;
        match link_type {
            LinkTypes::Unit => {
                let (state, flags) = convert_tag(l.tag)?;
                units.push(UnitInfo{
                    hash: target,
                    state,
                    flags,
...
fn build_tree(tree: &mut Tree<Content>, node: usize, path: Path) -> ExternResult<()>{
    for path in path.into_typed(ScopedLinkType::try_from(LinkTypes::Tree)?).children_paths()? {
        let v = path.as_ref();
        let units = get_entry_hashes(&path)?;
...
pub fn _get_tree(root_path: Path) -> ExternResult<Tree<Content>> {
    let units = get_entry_hashes(&root_path)?;
    let val = Content {
        name: String::from(""),
        units,
    };
